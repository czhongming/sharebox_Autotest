...
from . import air_cv
from .air_cv import cv2
from .air_cv.transform import TargetPos

from .air_cv.template_matching import TemplateMatching
from .air_cv.keypoint_matching import KAZEMatching, BRISKMatching, AKAZEMatching, ORBMatching
from .air_cv.keypoint_matching_contrib import SIFTMatching, SURFMatching, BRIEFMatching
...
def loop_find(query, timeout=ST.FIND_TIMEOUT, threshold=None, interval=0.5, intervalfunc=None):
    ...
    while True:
        # TODO
        screen = air_cv.imread(os.path.join(os.path.dirname(__file__), "../screen.png"))
        if screen is None:
...
def try_log_screen(screen=None):
    ...
    air_cv.imwrite(filepath, screen, ST.SNAPSHOT_QUALITY)
    return {"screen": filename, "resolution": air_cv.get_resolution(screen)}


class InvalidMatchingMethodError(Exception):
    pass
...
class Template(object):
        ...
        for method in ST.CVSTRATEGY:
            ...
            if func is None:
                raise InvalidMatchingMethodError(
                    "Undefined method in CVSTRATEGY: '%s', try 'kaze'/'brisk'/'akaze'/'orb'/'surf'/'sift'/'brief'"
                    " instead." % method)
            else:
    ...
    def _try_match(func, *args, **kwargs):
        ...
        except air_cv.NoModuleError as err:
            # print(
            #     "'surf'/'sift'/'brief' is in opencv-contrib module. You can use 'tpl'/'kaze'/'brisk'/'akaze'/'orb' in"
            #     " CVSTRATEGY, or reinstall opencv with the contrib module.")
            return None
        except air_cv.BaseError as err:
            print(repr(err))
            return None
        else:
            return ret
    ...
    def _imread(self):
        return air_cv.imread(self.filepath)
    ...
    def _find_keypoint_result_in_predict_area(self, func, image, screen):
        ...
        image_wh, screen_resolution = air_cv.get_resolution(image), air_cv.get_resolution(screen)
        xmin, ymin, xmax, ymax = Predictor.get_predict_area(self.record_pos, image_wh, self.resolution,
                                                            screen_resolution)
        # crop predict image from screen
        predict_area = air_cv.crop_image(screen, (xmin, ymin, xmax, ymax))
    ...
    def _resize_image(self, image, screen, resize_method):
        ...
        if not self.resolution:
            return image
        screen_resolution = air_cv.get_resolution(screen)
